...
export default function TimelineSection() {
  ...
  return (
    <section className="relative py-20 overflow-hidden bg-[#F2E8DC] border-t-4 border-b-4 border-[#5C3A21]/20">
      ...
      {/* --- BACKGROUND HÍBRIDO ILUSTRADO --- */}
      <div className="absolute inset-0 pointer-events-none select-none overflow-hidden">
        {/* Textura Editorial (pontos) */}
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(#5C3A21 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>
        
        {/* Iluminação Dourada Lateral */}
        <div className="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-[#E7C873]/30 to-transparent blur-3xl"></div>
        
        {/* Paisagem Estilizada ao Fundo */}
        <svg className="absolute bottom-0 w-full h-48 opacity-20" viewBox="0 0 1440 200" preserveAspectRatio="none" fill="#6B8E23">
          <path d="M0,160 C200,100 400,180 600,120 C800,60 1000,140 1200,80 C1300,50 1400,90 1440,100 L1440,200 L0,200 Z"></path>
        </svg>
        ...
        <div className="absolute top-10 left-10 w-24 h-24 border-2 border-[#8A5A3C]/20 rounded-full flex items-center justify-center rotate-12">
          <div className="w-8 h-8 border-l-2 border-r-2 border-[#8A5A3C]/30"></div>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 mb-12 text-center">
        ...
        <p className="text-lg text-[#8A5A3C] max-w-2xl mx-auto">
            ...
            {timelineData.map((item, index) => (
              ...
              >
                ...
                <div className={`relative group w-48 ${index % 2 === 0 ? 'mb-8' : 'mt-8 order-3'}`}>
                  <div className={`
                    relative p-4 bg-white border-2 border-[#5C3A21] rounded-xl shadow-[4px_4px_0px_0px_rgba(92,58,33,0.2)]
                    transition-all duration-300 group-hover:shadow-[6px_6px_0px_0px_rgba(92,58,33,0.4)] group-hover:-translate-y-1
                    ${item.type === 'value' ? 'bg-[#F2E8DC]/80 border-dashed' : 'bg-white'}
                  `}>
                    {item.type === 'history' ? (
                      <>
                      ...
                      <div className="text-center py-2">
                        <span className="text-[#E7C873] font-bold text-xs uppercase tracking-widest">Valor</span>
                        <h3 className="text-[#6B8E23] font-bold mt-1 text-md">{item.value}</h3>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className={`
                  ...
                  border-2 border-[#5C3A21] bg-[#F2E8DC]
                  group hover:scale-110 transition-transform duration-300
                  ${item.type === 'history' ? 'shadow-[0_0_15px_rgba(231,200,115,0.3)]' : 'border-[#6B8E23] bg-white'}
                `}>
                  ...
                  {/* Glow no Hover */}
                  <div className="absolute inset-0 rounded-full bg-[#E7C873]/0 group-hover:bg-[#E7C873]/30 transition-all duration-300 blur-md"></div>
                </div>

                {/* Espaçador para layout alternado */}
                <div className={`h-8 ${index % 2 === 0 ? 'order-3' : 'order-1'}`}></div>
              </div>
            ))}
          </div>
        </div>
